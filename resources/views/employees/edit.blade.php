   ...
   <div class="row">
      <div class="col-md-8">
         <div class="card">
            ...
            <div class="card-body">
               <form id="create" action="{{ route("employees.update", $data->id) }}" method="POST">
                  @csrf
                  @method("PATCH")
                  <div class="form-group">
                     <label>{{__("First name")}}</label>
                     <input type="text" name="first_name" value="{{ $data->first_name }}" class="form-control {{ $errors->has('first_name') ? ' is-invalid' : '' }}">
                     <p class="invalid-feedback">{{ $errors->first('first_name') ?? '' }}</p>
                  </div>
                  <div class="form-group">
                     <label>{{__("Last name")}}</label>
                     <input type="text" name="last_name" value="{{ $data->last_name }}" class="form-control {{ $errors->has('last_name') ? ' is-invalid' : '' }}">
                  ...
                  <div class="form-group">
                     ...
                     <select name="company" class="form-control {{ $errors->has('company') ? ' is-invalid' : '' }}">
                        ...
                        @foreach ($companies as $id => $name)
                           <option {{ (($id == $data->company) ? 'selected' : '' ) }} value="{{ $id }}">{{ $name }}</options>
                        @endforeach
                     </select>
                  ...
                  <div class="form-group">
                     ...
                     <input type="text" name="phone" value="{{ $data->phone }}" class="form-control {{ $errors->has('phone') ? ' is-invalid' : '' }}">
